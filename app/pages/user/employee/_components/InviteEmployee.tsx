...
import { InviteEmployeeSchema } from '@/app/lib/type';
import { inviteEmployeeSchema } from '@/app/lib/validationSchemas';
...
import { useInviteEmployee } from '../_features/hook';
import { useFetchDepartments } from '../../department/_features/hook';

interface Props {
	setOpen: React.Dispatch<React.SetStateAction<boolean>>;
}

const InviteEmployee: React.FC<Props> = ({ setOpen }) => {
	...
	} = useForm<InviteEmployeeSchema>({
		resolver: zodResolver(inviteEmployeeSchema),
	});

	const {
		mutateAsync: inviteEmployee,
		isSuccess: isCreateSuccess,
	} = useInviteEmployee({ axios });

	const onSubmit = async (data: InviteEmployeeSchema) => {
		try {
			await inviteEmployee({ ...data, businessUserId });
		} catch (error: any) {
		...
		<div>
			...
			>
				...
				<div className="items-center text-center mt-5">
					<h2 className="text-md font-semibold text-black mb-4">Invite a new employee</h2>
					<p>Fill in the details below to invite a new employee</p>
				</div>
