...
import { ReviewAdminKycSchema } from '@/app/lib/type';
import { reviewAdminKycSchema } from '@/app/lib/validationSchemas';
...
import { useReviewAdminKyc } from '../_features/hook';

interface Props {
  setOpen: React.Dispatch<React.SetStateAction<boolean>>;
  adminUserId: string;
}
const ReviewBusinessKyc: React.FC<Props> = ({ setOpen, adminUserId }) => {
  ...
  } = useForm<ReviewAdminKycSchema>({
    resolver: zodResolver(reviewAdminKycSchema),
  });

  const {
    mutateAsync: reviewBusiness,
    isSuccess: isReviewSuccess,
  } = useReviewAdminKyc({ axios });

const onSubmit = async (data: ReviewAdminKycSchema) => {
    try {
      await reviewBusiness({ ...data, adminUserId, businessUserId });
    } catch (error: any) {
  ...
  useEffect(() => {
    if (isReviewSuccess) {
      queryClient.invalidateQueries({ queryKey: ["admins-kyc"] });
      toast.success(`Admin kyc reviewed successfully`);
      setOpen(false);
    }
    ...
    <div>
      ...
      >
        ...
        <div className="items-center text-center mt-5">
          <h2 className="text-md font-semibold text-gray-700 mb-4">Review Admin Kyc</h2>
        </div>
