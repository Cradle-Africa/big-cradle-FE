...
interface Props {
    uniqueId: string;
    deletingDataPoint: boolean;
    setDeletingDataPoint: (value: boolean) => void;
    dataPointName: string;
}

const DeleteDataPoint: React.FC<Props> = ({ deletingDataPoint, uniqueId, setDeletingDataPoint, dataPointName }) => {
    const menuRef = useRef<HTMLDivElement>(null);
    const queryClient = useQueryClient();

    const { mutateAsync: deleteDataPoint, isSuccess, isPending } = useDeleteDataPoint({ axios });

    const onSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            await deleteDataPoint({ id: uniqueId });
        } catch (error: any) {
    ...
    }, [isSuccess, queryClient, setDeletingDataPoint]);

    if (!deletingDataPoint) return null;
